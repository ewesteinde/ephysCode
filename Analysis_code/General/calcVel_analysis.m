function [ velocityOut , accumulatedPositionOut ] = calcVel_analysis( ficTracBallPosition , sampleRate , FicTracRate, yaw)
...
filteredPosition = smoothdata(downsampled_cleanedPos,'loess',15);
...
% new derivative function has a better signal to noise ration than gradient
% & similar or better temporal fidelity, takes longer to compute & so less
% ideal for velocity visualization during an experiment

%velocity = rdiff(filteredPosition, 1/30);
...
% smooth velocity
%velocityOut = smoothdata(interpVel,'loess',10);
