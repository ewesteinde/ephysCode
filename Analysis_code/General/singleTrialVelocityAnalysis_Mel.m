function [smoothed] = singleTrialVelocityAnalysis_Mel(data, sampleRate)
...
%% Downsample the position data to match FicTrac's output
    ...
    downsampled.Intx = resample(data( :, settings.raw.x ),25,sampleRate); %For a 1000 rate acquisition frame rate from the NiDaq, downsampling to 25 Hz equals taking 1 every 40 frames
    downsampled.Inty = resample(data( :, settings.raw.y ),25,sampleRate); %For a 1000 rate acquisition frame rate from the NiDaq, downsampling to 25 Hz equals taking 1 every 40 frames  
    downsampled.angularPosition = resample(data( :, settings.raw.yaw ),25,sampleRate);
