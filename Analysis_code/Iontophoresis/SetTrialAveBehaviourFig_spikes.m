function SetTrialAveBehaviourFig_spikes(rootDir, pulse_window, sampRate)
    arguments
        rootDir string
...
for ff = 1:folderNum
    ...
    for t = 1:length(processed_behaviourData)
        ...
        for p = 1:length(pulseLengths)
            ...
            try
                ...
                for i = 1:numPulses
                    ScOAve1 = ScOAve1 + processed_trialData{t}.fRate_sec(pulses.windowStart(i):pulses.windowEnd(i));
                    plot(time, processed_trialData{t}.fRate_sec(pulses.windowStart(i):pulses.windowEnd(i)))
                    hold on
                end
            catch
                ScOAve1 = zeros(pulse_window* 2 *sampRate + 1,1);
    ...
    %% side
            ...
            saveas(gcf,fullfile(folder,'figures',[num2str(pulse),'msPulse_fRate_expAve.fig']))
        end
    end
end
end
