function [rho, theta, idx_windows, f, g] = PFL2_3_headingDist(window, minVel, processed_behaviourData, sampRate, plotFig)
...
if window > length(processed_behaviourData.vel_for)
    window = length(processed_behaviourData.vel_for); 
end
count = 1; 
...
for i = 1:1000:length(processed_behaviourData.angle) - window + 1
    idx = i:i+window-1; 
