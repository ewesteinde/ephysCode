...
%%
function [] = PFL2_3_behaviourVSactivity_lag(t, lag, prefHead, step, range, date, cell_num, trial, processed_trialData, processed_behaviourData, fileName,iStart, iEnd) 

%% 
name = strcat(date,', ',cell_num,', ',trial,' prefAngle: ',num2str(prefHead),' BinSize: ',num2str(range), ' Lag: ',num2str(lag));
...
Vs = processed_behaviourData.vel_side(iStart:iEnd)';
Vy = processed_behaviourData.vel_yaw(iStart:iEnd)';


if lag < 0 % negative lag means neural activity precedes behaviour
    fRate = fRate(1:end+lag);
    Vm = Vm(1:end+lag);
    angle = angle(1:end+lag);
    
    Vf = Vf(abs(lag)+1:end);
    Vs = Vs(abs(lag)+1:end);
    Vy = Vy(abs(lag)+1:end);
elseif lag > 0 % positive lag means neural activity follows behaviour
    fRate = fRate(lag+1:end);
    Vm = Vm(lag+1:end);
    angle = angle(lag+1:end);
...
for head = headings
    ...
    fRateinHead = fRate(index);
            ...
            edges = [min(VfinHead(:,1)):step:max(VfinHead(:,1))]; 
            ...
            edges = [min(VyinHead(:,1)):10:max(VyinHead(:,1))]; 
...
edges = [min(speed):step:max(speed)];
    ...
    tempFR = accumarray(bin+1, fRate, [length(edges) 1]);
...
if strcmp(keep, 'y')
    cd(fileName) 
    savefig(g,strcat('ActivityVsVelocity_splitHead_lag_',num2str(lag),'.fig'))
    savefig(k,strcat('ActivityVsVelocity_allHead_lag_',num2str(lag),'.fig'))
